...

{
  ...
  config = mkIf cfg.enable (mkMerge [{
    ...
    systemd.services.dnscrypt-proxy2 = {
      description = "dnscrypt-proxy daemon, 2.x";
      documentation = [ "man:dnscrypt-proxy(8)" ];

      before = [ "nss-lookup.target" ];
      after = [ "network.target" ];
      requires = [ "dnscrypt-proxy2.socket "];

      serviceConfig = {
        NonBlocking = "true";
        ExecStart = "${pkgs.dnscrypt-proxy2}/bin/dnscrypt-proxy ${toString daemonArgs}";
    ...
    systemd.services.dnscrypt-proxy2.after = [ "apparmor.service" ];

    security.apparmor.profiles = singleton (pkgs.writeText "apparmor-dnscrypt-proxy2" ''
      ${pkgs.dnscrypt-proxy2}/bin/dnscrypt-proxy {
        ...
        ${getLib pkgs.dnscrypt-proxy2}/lib/dnscrypt-proxy/libdcplugin*.so mr,
